...
public:
    ...
    inline bool add_correction(const RefConstVec& s, const RefConstVec& y)
    {
        ...
        Scalar ys = m_s.col(loc).dot(m_y.col(loc));
        if(abs(ys)<1e-7)
            return false;
        m_ys[loc] = ys;
    ...
    inline bool apply_Hv(const Vector& v, const Scalar& a, Vector& res)
    {
        res.resize(v.size());
        for(int i=0; i<m_ys.size(); i++)
            if(abs(m_ys[i])<1e-7)
