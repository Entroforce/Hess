...
100 continue
  ...
  subroutine aq_vec (coord, srad, numat, cosurf, nps, nar_csm, nsetf, &
    ...
    !
    !.. Local Arrays ..
    double precision, dimension(4, 300, 2) :: finel
...
!
  ...
  subroutine aq_mult_int (ind, n, psi, p, x0, y0, z0, y_norm, pmn, c, n3, &
    ...
    complex (kind=dcmplx_kind) :: tc, temp_complex
...
!
    ...
    do i = 1, n
      ...
      do j = 0, p
        ...
        do k = 1, j
          temp_complex = psi(k,j)
          tc = tc + y_norm(k, j) * pmn(k, j) * d**j &
               & * temp_complex * Exp (Cmplx (0.d0, k*phi, dcmplx_kind))
        end do
        
        !write(*, "(A, F16.6, F16.6)"), "k: ", Real(tc), Aimag(tc)
        
        !write(*, "(A, F16.6, F16.6)"), "tc psi", Real (tc) , Real (psi(0, j))
        !write(*,"(A,F16.6, F16.6, F16.6, F16.6)") "psi", Real(psi(0, j))
        temp_complex = psi(0,j);
        t = t + 2.d0 * Real (tc) + pmn(0, j) * d**j * Real (temp_complex)
!        if(ind(i) == 2) then
!            write(*,"(A)") "two"
!            write(*, "(F16.6, F16.6, F16.6, F16.6, I10, F16.6)") Real(tc), pmn(0, j), Real(psi(0,j)) &
!            , d**j, j, t
!        end if
      end do
      
      !write(*, "(A,F16.6, I10)") "t, ind: ",t, ind(i)
!      if (ind(i) == 2) then
!        write(*,"(A)") "here 2"
!        write(*,"(F16.6, F16.6, F16.6, F16.6, F16.6, F16.6, F16.6, F16.6, F16.6)") Real(tc), pmn(0,0), pmn(0,1), pmn(0,2) &
!        , pmn(0,3), d, &
!        Real (psi(0, 0)), Real (psi(0, 1)), Real (psi(0, 2)), Real (psi(0, 3))
!      end if 
      r(ind(i)) = r(ind(i)) + t

    end do
    
    !write(*, "(A,F16.6, F16.6, F16.6, F16.6, F16.6)") "R: ", r(1), r(2), r(3), r(4), r(5)
...
!
    ...
    !write(*, "(A, F16.6, F16.6, F16.6, F16.6)") "before new x", x(1), x(2), x(3), x(4)
    
    if(new_x) then        ! initial guess
      call precondition_solve (m, x, b, n)
    end if
    
    !write(*, "(A, F16.6, F16.6, F16.6, F16.6)") "after new x", x(1), x(2), x(3), x(4)

    if (n > na2max) then
        !write(*, "(A, F16.6, F16.6, F16.6, F16.6)") "before na2max", q(1), q(2), q(3), q(4)
      call afmm (cosurf, 4, n, x, q, n, aq_dir_int, aq_mult_int)
      !write(*, "(A, F16.6, F16.6, F16.6, F16.6)") "after na2max", q(1), q(2), q(3), q(4)
      q = q + a_diag * x
      !write(*, "(A, F16.6, F16.6, F16.6, F16.6)"), "a_diag: ", a_diag(1), a_diag(2), a_diag(3), a_diag(4)
...
!   write(26,*) ' Tol = ', t
!
    !write(*, "(A, F16.6, F16.6, F16.6, F16.6)") "before iter", x(1), x(2), x(3), x(4)
    ! error in this loop
    do i = 1, max_iters

      new_iteration = .true.
      !write(*, "(A, F16.6, F16.6, F16.6, F16.6)") "before z r", z(1), z(2), r(1), r(2)
      call precondition_solve (m, z, r, n)
      !write(*, "(A, F16.6, F16.6, F16.6, F16.6)") "after z r", z(1), z(2), r(1), r(2)
      ...
      !write(*, "(A, F16.6, F16.6, F16.6, F16.6, F16.6)") "rho p q", rho, p(1), p(2), q(1), q(2)
      x = x + alpha * p
      r = r - alpha * q
...
!
   ...
   !     write (iw, *) i, " microiterations, err = ", tt
    ...
    !write(*, "(A, F16.6, F16.6, F16.6, F16.6)") "after iter", x(1), x(2), x(3), x(4)
  ...
  subroutine addfckz
    ...
    !
    !.. Local Scalars ..
    integer :: i, iat, im, j, ii, nao, nnn, ierr, kk
 ...
 !   write(*, "(A,F16.6,F16.6,F16.6,F16.6)") "before1: ", qscnet(1, 2), qscnet(2, 2), qscnet(3, 2), qscnet(4, 2)
...
!    write(*, "(A,F16.6,F16.6,F16.6,F16.6)") "before2: ", qscnet(1, 2), qscnet(2, 2), qscnet(3, 2), qscnet(4, 2)
    do i = 1, nps
      iat = iatsp(i)
...
!    write(*, "(A,F16.6,F16.6,F16.6,F16.6)") "after: ", qscnet(1, 2), qscnet(2, 2), qscnet(3, 2), qscnet(4, 2)
    ediel = ediel * fcon / 2
      ...
      do i = 1, numat
        ...
        if (nao == 0) then ! S - element
          ...
          do j = 1, nps
            ...
            !write(*, "(F16.6, F16.6, F16.6, F16.6)") qscnet(1, 2), qscnet(2, 2), qscnet(3, 2), qscnet(4, 2)
