subroutine atomrs (lused, ioptl, ires, n1, io, uni_res, first_res)
    use molkst_C, only: natoms, numat, line, keywrd, ncomments
    use chanel_C, only: iw, log
    ...
    if (ico /= 0 .and. .not. UNK .and. i > 1 .and. .not. first_res .and. loop == 1) then
      if (nbonds(ico) /= 4 .or. nbonds(jofco) /= 2 .or. i /= 3) then  ! Exclude end group being -CH2OH instead of -COOH
        if(odd_h .and. k /= size_mres) then       
          ...
          write(line,"(a)")"                      atom number of "
          write(iw,'(a)')trim(line)
          !if (log) write(ilog,'(a)')trim(line)
          write(line,"(a)")"      Residue number  an atom in res.   Type     No of extra hydrogen atoms"
          write(iw,'(a)')trim(line)
          !if (log) write(ilog,'(a)')trim(line)
        end if
        if (k /= size_mres) then
          write(line,"(10x, i4,i17,9x,a,i17)")ires, iatom, loc_tyres, nh - mres(5,k) 
          write(iw,'(a)')trim(line)
          !if (log) write(ilog,'(a)')trim(line)
        end if
      end if
