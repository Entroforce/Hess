...
contains
  ...
  subroutine afmm (coord, nc, n, q, res, res_size, dir_int, mult_int, &
    ...
    !write(*, "(A, F16.6, F16.6, F16.6, F16.6)"), "1 res: ", res(1), res(2), res(3), res(4)
    do i = 1, nbox
      if (b(i)%n_childs == 0) then
    ...
    !write(*, "(A, F16.6, F16.6, F16.6, F16.6)"), "2 res: ", res(1), res(2), res(3), res(4)
    
    !
    !       Step 6. Calculate the potential at each charge in a box from
    !       ======  it's local expansion
    !
!    write(*, "(A, F16.6, F16.6, F16.6, F16.6, F16.6, F16.6)"), "psipsi: ", &
!    Real(b(293)%psi(0,1)), Aimag(b(293)%psi(0,1)), Real(b(293)%psi(0,2)), Aimag(b(293)%psi(0,2)), &
!    Real(b(293)%psi(0,3)), Aimag(b(293)%psi(0,3))
!    write(*, "(A, F16.6, F16.6, F16.6, F16.6)"), "B: ",y_norm(1,1), y_norm(1,2), y_norm(1,3), y_norm(1,4)
    do i = 1, nbox
      if (b(i)%n_childs == 0) then
...
!    write(*, "(A, F16.6, F16.6, F16.6, F16.6)"), "3 res: ", res(1), res(2), res(3), res(4)
  end subroutine afmm
...
!            if (j == 293) then
!                write(*, "(A, F16.6, F16.6)"), "afmm; ", REAL(b(j)%psi(m1, n1)), AIMAG(b(j)%psi(m1, n1))
!            end if
          end do
        end do
