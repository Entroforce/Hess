...
!
      ...
      !
        ...
        read (iden) (nbonds(i), i=1, numat)
        read (iden) ((ibonds(j, i), j=1, 9), i=1, numat)
      !
...
1010    continue
      !
      !
        write (iw,"(//10x,a)") " FILE " // trim(density_fn) // " EXISTS, BUT IS FAULTY"
        call mopend ("DENSITY file is faulty")
        return
      !
      !
1020    continue
      !
          write (iw,"(//10x,a)") " OLDENS FILE FOR " // trim(density_fn) // " IS CORRUPT"
          call mopend ("DENSITY file is corrupt")
          return
      end if
     !
1030  write (iw,*) " Cannot write density matrix to " // density_fn
      call mopend (" Cannot write density matrix to file")
      return
   !
