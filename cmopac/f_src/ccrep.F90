...
!-----------------------------------------------
      integer :: nt, ig, i, j
    !  logical :: opend
      real(double) :: alpni, alpnj, enuc, abond, fff, scale, eni, enj, ax
!-----------------------------------------------
...
!
     ...
     !
        ...
        if (.not. method_mndod) then
          if (method_pm6 .or. method_pm7) then
...
!
   ...
   !   inquire(unit=44, opened=opend) 
   !   if (.not. opend) open(unit = 44, name = "ccrep.txt")
   !   write(44,'(2F10.3)') (enuclr - enuc)/(tore(ni)*tore(nj))
      return  
