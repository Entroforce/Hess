            ...
            if (j /= 0) then
              ...
              do
                ...
                if (k == ires) then
                  ...
                  if (first) then
                    first = .false.
                    write(iw,'(/,a)') "      Ligand names that have been changed"
                    write(iw,'(a)') "      Residue No.  Calc'd name   XENO name"
                  end if
                    change_no = change_no + 1
                    write(iw,'(i3,i10,2x,a1,7x,a3,9x,a3)')change_no, ires, num, het, line(2:4)
                    het = line(2:4)
                  exit
                end if
              end do
          ...
          if (nat(i) /= 1 .or. nbonds(i) /= 1) then
            if (ncomments < max_comments .and. index(keywrd, " RESID") /= 0) then
              ...
              write(all_comments(ncomments),'(a,i5)') &
                 "*REMARK   3   "//het//" = "//het_group(:47)//"res: ", ires
              !if (log) then
              !   write(ilog,'(a,i5)') &
              !   "Hetero-group   "//het//" = "//het_group(:47)//"res: ", ires
...
subroutine moiety (iopt, lused, istart, new)
   ...
   !
   !.. Parameters ..
    integer, parameter :: natomr = 800
   !
   !.. Local Scalars ..
    integer :: i2, i3, iatom, j, j2, k, l, ninbit, nlive
   !
   !.. Local Arrays ..
    integer, dimension (200) :: live
