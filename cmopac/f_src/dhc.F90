subroutine dhc (p, pa, pb, xi, nat, if, il, jf, jl, dener, mode, isgood)
    use molkst_C, only : numcal, cutofp, id, uhf
    use helect_I
    implicit none
    integer, intent (in) :: if, il, jf, jl, mode, isgood
    ...
    if (isgood == 1) then
      write(*,"(A)") "Reloaded dhc!"
      icalcn = 0
      w(1:2026) = 0.0
      wk(1:2026) = 0.0
      e1b(1:45) = 0.0
      e2a(1:45) = 0.0
      call fock2 (f, p, pa, w, w, wk, i, nfirst_loc, nlast_loc, 1, isgood)
      return
    end if
    ...
    if (uhf) then
      ...
      i = -2
      call fock2 (f, p, pb, w, w, wk, i, nfirst_loc, nlast_loc, 1, isgood)
