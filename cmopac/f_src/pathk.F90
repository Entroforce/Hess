...
!
        ...
        factor = abs(escf)
        num2 = char(max(ichar("0"), ichar("0") + int(log10(factor)) - 1))
        num3 = char(max(ichar("0"), ichar("0") + int(log10(4.184d0*factor)) - 1))        
...
!
  ...
  subroutine write_path_html
    ...
    line = line(i+1:)
    write(iprt,"(a)")" <a href='"//trim(line)//"'   target=""_blank"">Read Script</a>"
    write(iprt,"(a)")" <a href='javascript:jmolScript(""script \"""//trim(line)//"\"";"")'>Run Script</a>" 
    write(iprt,"(a)")"<br>"
    write(iprt,"(a)")"</td>"
