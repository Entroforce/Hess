...
!            ibonds(j,i) = atom numbers of atoms attached to atom i (there are nbonds(i) of these)
    use molkst_C, only : numat, l11, l21, l31, id
    use mozyme_C, only : nijbo
    use common_arrays_C, only : nat, coord, tvec, nbonds, ibonds
...
!
    ...
    use molkst_C, only : numat, method_pm6, method_pm7
    implicit none
    integer :: i, j
    double precision :: correction, sum
    double precision, external :: nsp2_atom_correction
    if ( .not. (method_pm6 .or. method_pm7)) then
